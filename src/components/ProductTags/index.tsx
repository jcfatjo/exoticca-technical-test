...
import { GROUP_TOURS_TOOLTIP_LABEL_TEXT, SOLO_TRAVELLER_TOOLTIP_LABEL_TEXT } from "@/components/ProductTags/ProductTags.constants.ts";
            ...
            <Flex className="flex-wrap items-end justify-start gap-1 overflow-hidden w-full h-[52px]">
                {isGroupTours && (
                    <Tooltip hasArrow label={GROUP_TOURS_TOOLTIP_LABEL_TEXT} bg={"gray.200"} color={"black"}>
                        <Tag data-cy="product-tag" className="rounded-full w-fit h-fit uppercase hover:cursor-default" variant="outline">
                            <TagLabel data-cy="tag-label">Group tours</TagLabel>
                        </Tag>
                    </Tooltip>
                )}
                {isSoloTraveller && (
                    <Tooltip hasArrow label={SOLO_TRAVELLER_TOOLTIP_LABEL_TEXT} bg={"gray.200"} color={"black"}>
                        <Tag data-cy="product-tag" className="rounded-full w-fit h-fit uppercase hover:cursor-default" variant="outline">
                            <TagLeftIcon boxSize="12px" as={PersonIcon} />
                            <TagLabel data-cy="tag-label">Solo traveller</TagLabel>
                        </Tag>
                    </Tooltip>
                )}
                {tags.map((tag: ProductTag, index: number) => (
                    <Tooltip key={`tag-${index}`} hasArrow label={tag.description} bg={"gray.200"} color={"black"}>
                        <Tag data-cy="product-tag" className="rounded-full w-fit h-fit hover:cursor-default">
                            <TagLabel data-cy="tag-label">{tag.name}</TagLabel>
                        </Tag>
                    </Tooltip>
                ))}
            </Flex>
